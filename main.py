...
window=tkinter.Tk()
# window.title("차량 ")
# backgroundlabel=tkinter.Label(window, image=bgpt).grid(row=0, column=1)
# cw=tkinter.Tk()
# cw.mainloop()
#
# cw.title("학습 데이터 설정")
...
bgpt=tkinter.PhotoImage(file="background.png")
buyingdrop=[
    "매우 비쌈",
    "비쌈",
    "보통",
...
klabel=tkinter.Label(window, text="k_fold").grid(row=1, column=0)
kfoldinput=tkinter.Entry(window)
kfoldinput.bind("<Motion>", calc)
kfoldinput.grid(row=1, column=1)
# modelsetlabel=tkinter.Label(window, text="모델 종류 선택").grid(row=2, column=0)
# modelopt=tkinter.OptionMenu(window, modelopt, *modelsdrop)
# modelopt.config(width=12, font=('Helvetica', 12))
# modelopt.grid(row=5, column=1)
...
#
# model2opt=tkinter.StringVar(window)
# model2opt.set("옵션을 선택해 주세요.")
...
def modelcallback(*args):
    ...
    if models=="knn":
        ...
        def n2set(*args):
            global neigh
            # global n_neighbors
            neigh=int(ninput.get())
            n_neighbors=int(ninput.get())
            print(neigh)
            print(n_neighbors)
        nlabel=tkinter.Label(window, text="n_neighbors").grid(row=2, column=2)
        ninput = tkinter.Entry(window)
        ninput.bind("<Motion>", n2set)
        ninput.grid(row=2, column=3)
    else:
        nlabel = tkinter.Label(window, text="n_neighbors")
        ninput = tkinter.Entry(window)
        nlabel.grid_forget()
        ninput.grid_forget()
    # debuglabel1.configure(text=buyingopt.get())
...
def learn():
    ...
    accuracylabel.grid(row=3, column=1)

    blank=tkinter.Label(window, text="").grid(row=4, column=0)
    ...
    def predict():
        ...
        try:
            ...
            messagebox.showinfo("에러","모든 정보를 입력해 주세요.")

    bl2 = tkinter.Label(window, text="")
    bl2.grid(row=11, column=0)
    ...
    calcbutton.grid(row=12, column=0)

    progressbar=tkinter.ttk.Progressbar(window, maximum=100, mode="determinate")
    progressbar.grid(row=12, column=1)
    progressbar.step(10)

    resultlabel = tkinter.Label(window, text="", fg="blue")
    resultlabel.grid(row=13, column=0)
learnbutton = tkinter.Button(window, text="학습 시작", overrelief="solid", width=10, command=learn, repeatdelay=1000, repeatinterval=100)
learnbutton.grid(row=12, column=0)
